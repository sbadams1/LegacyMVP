...
function fixSecondPersonObjectPronounOverreach(text) {
  if (!isNonEmptyString(text)) return '';
  let s = text;

  // Food/animal/object pronouns that were incorrectly converted to "you"
  s = s.replace(/\bspare you\b/gi, 'spare them');
  s = s.replace(/\bate you raw\b/gi, 'ate them raw');

  // The “try you for the first time” artifact (oysters)
  s = s.replace(/\bleading you to try you\b/gi, 'leading you to try them');
  s = s.replace(/\btry you for the first time\b/gi, 'try them for the first time');

  // “get rid of you” (refers to oysters/food, not the user)
  s = s.replace(/\bget rid of you\b/gi, 'get rid of them');

  // If we still have “their brother … leading you …” that should be “your brother”
  s = s.replace(/\bTheir brother\b/g, 'Your brother');

  // Narrator drift after "Additionally,"
  s = s.replace(/\bAdditionally,\s*they\b/gi, 'Additionally, you');

  // Sentence-start capitalization fixes
  s = s.replace(/(^|[.!?]\s+)(your girlfriend)\b/g, (m, p1) => `${p1}Your girlfriend`);
  s = s.replace(/\bCurrently,\s*You\b/g, 'Currently, you');
...
function ensureSecondPerson(summary) {
  ...
  s = fixSecondPersonObjectPronounOverreach(s);
