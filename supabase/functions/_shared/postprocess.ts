// supabase/functions/_shared/postprocess.ts
// Single post-processing pipeline shared by:
// - rebuild-insights (HTTP function wrapper)
// - ai-brain end_session (direct import/call)
//
// This is a lift-and-shift of your rebuild-insights logic with minimal changes:
// - Gemini calls route through _shared/gemini.ts (via geminiGenerateContent)
// - jsonResponse/CORS stays in _shared/http.ts (wrappers handle HTTP)
// - Optional restriction to one conversation_id for end_session use.

import type { SupabaseClient } from "https://esm.sh/@supabase/supabase-js@2";
import { geminiGenerateContent, extractGeminiTextFromResponse, parseJsonSafely } from "./gemini.ts";
...
async function backfillSessionSummariesFromMemoryRaw(
  ...
  userId: string,
  opts: { only_conversation_id?: string | null } = {},
  ...
  if (opts.only_conversation_id) {
    q.eq("conversation_id", opts.only_conversation_id);
  }
...
export type PostProcessResult = {
  ...
  lite?: boolean;
  life_themes?: any;
  lifetime_profile?: any;
  insights_inserted?: number;
  error?: string;
  details?: any;
};
...
export async function runPostProcess(
  ...
  const conversationId = (args.conversation_id ?? null)?.trim() || null;
  const lite = typeof args.lite === "boolean" ? args.lite : true;
  ...
  try {
    const backfill = await backfillSessionSummariesFromMemoryRaw(supabase, userId, { only_conversation_id: conversationId });
    console.log(
    ...
    return { ok: false, error: "runPostProcess failed", details: String(e) };
  }
}
