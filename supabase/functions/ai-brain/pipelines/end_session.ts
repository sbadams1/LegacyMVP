...
// ---------------------------------------------------------------------------
 ...
 // 🔥 FIX: ensure full narrative is evaluated at least once
 const turns =
   startIdx === 0
     ? userTurns
     : userTurns.slice(startIdx, startIdx + 6);
   ...
   // TEMP: disable truncation for story capture while debugging heuristic misses.
   const stopAt = -1;
   const safeTexts = stopAt >= 0 ? texts.slice(0, stopAt) : texts;
   ...
   async function persistHeuristicStorySeedsV1(opts: {
     ...
     // Accept both names to stay compatible with Phase B caller.
       ...
       const userTurns: TranscriptTurnLite[] = (Array.isArray((opts as any)?.userTurns) ? (opts as any).userTurns : [])
        ...
        source: String(
          t?.source ?? (String(t?.role ?? "").trim().toLowerCase() === "user" ? "legacy_user" : "")
        ).trim(),
        text: String(t?.text ?? t?.content ?? "").trim(),
      }))
      // Only seed from user-origin turns to avoid capturing assistant recaps as "stories".
      .filter((t: any) =>
        t &&
        t.text &&
        (String(t.role ?? "").toLowerCase() === "user" || String(t.source ?? "").toLowerCase().includes("user"))
      );
 
      let picks = pickHeuristicStoryTurnsV1(userTurns as any);
      ...
      if ((!Array.isArray(picks) || picks.length === 0) && userTurns.length >= 2) {
        ...
        if (isValidStoryBlockV1(fallbackText, fallbackReceiptIds)) {
          picks = [{
            idx: 0,
            title: deriveStoryTitleV1(fallbackText),
            text: fallbackText,
            receipt_ids: fallbackReceiptIds,
          }];
        }
      }
      ...
      if (!Array.isArray(picks) || picks.length === 0) {
       const debugOut = {
         ...
         user_chars: userTurns.map((t) => t.text).join(" ").trim().length,
         first_turn_preview: String(userTurns?.[0]?.text ?? "").slice(0, 160),
       };
       console.warn("END_SESSION: story seed picks empty", {
         conversation_id,
         user_turns: debugOut.user_turns,
         user_chars: debugOut.user_chars,
         first_turn_preview: debugOut.first_turn_preview,
       });
  ...
  for (const p of picks) {
    ...
    if (!isValidStoryBlockV1(text, storyReceiptIds)) {
     ...
     if (storyReceiptIds.length < 2 && text.length >= 600) {
      ...
      // (This pipeline is allowed to run in environments where the Phase B worker / Gemini keys are missing.)
      let aiHandle: string | null = null;
      try {
        aiHandle = await storyHandleTitleGeminiV1(text);
      } catch (e) {
        // Non-fatal: fall back to heuristic handle generation.
        aiHandle = null;
        console.warn("END_SESSION: story handle gemini call failed (non-fatal):", (e as any)?.message ?? e);
      }
      const title = enforceStorySeedTitleV1(
        rawTitle,
        text,
        (aiHandle && isValidStoryHandleV1(aiHandle) ? aiHandle : null) ?? storyHandleTitleV1(rawTitle, text),
      );
    const synopsis = oneLinerFromStoryTextV1(text);
    const storyText = clampString(text, 4000);
    const oneLiner = clampString(synopsis || text, 280);
    const seedTextPayload = JSON.stringify({
      story: storyText,
      one_liner: oneLiner,
         ...
         if (seedId) {
           ...
           if (!seedErr) {
             ...
             const validSeedId =
               ...
               /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(seedId);
             if (validSeedId) {
               try {
                ...
                const { error: recallUpsertErr } = await client
                   .from("story_recall")
                  .upsert(recallPayload, { onConflict: "user_id,story_seed_id" });

                // If the schema has a different unique constraint (or none), fall back to insert.
                 if (recallUpsertErr) {
                   const { error: recallInsertErr } = await client
                     .from("story_recall")
                     .insert(recallPayload);
                   if (recallInsertErr) {
                     console.warn("END_SESSION: story_recall insert failed (non-fatal):", {
                       upsert_code: (recallUpsertErr as any)?.code ?? null,
                       upsert_message: (recallUpsertErr as any)?.message ?? String(recallUpsertErr),
                       insert_code: (recallInsertErr as any)?.code ?? null,
                       insert_message: (recallInsertErr as any)?.message ?? String(recallInsertErr),
                     });
                    if (recallErrors.length < 3) {
                      recallErrors.push({
                        step: "story_recall.insert",
                        code: (recallInsertErr as any)?.code ?? null,
                        message: String((recallInsertErr as any)?.message ?? recallInsertErr),
                      });
                    }
                   }
                 }
                 ...
                 if (recallErrors.length < 3) {
                   recallErrors.push({ step: "story_recall.exception", code: null, message: String((e as any)?.message ?? e) });
                 }
                }
               } else {
                 console.warn("END_SESSION: skipping story_recall write because seedId is invalid", {
                   seedId,
                   title: seedPayload?.title ?? null,
                   conversation_id,
                 });
                 if (recallErrors.length < 3) {
                   recallErrors.push({
                     step: "story_recall.skipped_invalid_seed",
                     code: null,
                     message: `invalid seedId: ${String(seedId ?? "")}`,
                   });
                 }
                }
...
if (isPhaseBEnabled()) {
  ...
  try {
    ...
    if (storiesEligible) {
      ...
      (payload as any).phase_a_story_seeds = {
        ...phaseAStorySeeds,
        user_turns: userTurnsLite.length,
        user_chars: userTurnsLite.map((t) => t.text).join(" ").trim().length,
      };

      (payload as any).phase_a_story_seeds_debug =
        (phaseAStorySeeds as any)?.__phase_a_story_debug ?? ({} as any);

      endSessionTrace.push({
        step: "phase_a_story_seeds",
        ms: 0,
        meta: {
          ...(payload as any).phase_a_story_seeds,
          debug: (payload as any).phase_a_story_seeds_debug,
        },
      });
    }
  } catch (err) {
    ...
    endSessionTrace.push({
      step: "phase_a_story_seeds_failed",
      ms: 0,
      meta: {
        error: String((err as any)?.message ?? err),
        user_turns: userTurnsLite.length,
